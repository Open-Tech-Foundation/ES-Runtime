// Generate a small mock YAML document for benching
let yamlDoc = `catalog:\n`;
for (let i = 0; i < 50; i++) {
  yamlDoc += `  - id: bk${i}
...
// Deno, which have no built-in YAML parser. Holding Bun to a JS library it does
// not need would understate it by roughly 2x.
let parse = null;
try {
