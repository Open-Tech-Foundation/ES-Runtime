// Postgres QPS on esrun: @opentf/esrun-postgres, pool of 100.
...
const WARMUP_S = Number(env.QPS_WARMUP ?? 3);
const db = await connect(env.PG_URL, { driver: postgres, pool: { max: 100 } });

async function one() {
  // Rows is an async iterable, not an array: drain it to count and keep.
  const rows = [];
  for await (const r of await db.query(QPS_QUERY)) rows.push(r);
