...
| `symlink(target, path, opts?)` | `(target, path, { type? }) => Promise<void>` | Creates a symlink at `path` holding `target`. `ERR_ALREADY_EXISTS` if `path` is taken. `type` is `"file"`/`"dir"`, and only Windows asks. | `await symlink("../pkg", "./node_modules/pkg")` |
