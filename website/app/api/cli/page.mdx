...
| `--deny-all` | Run with no host access at all. Cannot be combined with `--deny-<name>`. See [Denying capabilities](#denying-capabilities). |
...
`--deny-all` **or** one or more `--deny-<name>` — passing both is an error, since
`--deny-all` is exactly the union of the eight.
...
<Callout type="note" title="--deny-all runs a single file">
  The entry file is read before the runtime exists, so `--deny-all` still runs
  what you named. It includes `--deny-imports`, though, so a fully denied run
  cannot load a second file. Query the policy from JS with
