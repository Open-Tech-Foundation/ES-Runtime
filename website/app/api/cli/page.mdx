...
`run` and `env` can be granted narrowed to a comma-separated list instead of
whole:
...
<Callout type="note" title="A scoped grant is still a grant">
  ...
  over. An unlisted program throws `ERR_PERMISSION_DENIED` ("you have `run`, but
  not this one"), where `--deny-run` throws `ERR_CAPABILITY_DENIED` ("you never
  had `run`").
</Callout>

Scoping the other six is not implemented, and a value on them is rejected rather
than ignored.
...
| --- | --- |
| `--allow-net=example.com` | Scoping `net` is not implemented; the value would be ignored |
