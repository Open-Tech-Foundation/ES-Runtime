...
## memoryUsage and uptime — reporting on yourself
...
<Callout type="note" title="No capability, and no rss">
  Both report only what the caller could discover about itself anyway — its
  ceiling by allocating until it is stopped, its age by counting — which is the
  same rule `platform` and `args` follow.

  The process's resident set and CPU are about the *other* agents in the process,
  so they are not here. They are `metrics().process` in
