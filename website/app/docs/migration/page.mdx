...
3. **Capability & Sandboxing Model**:
   - The esrun embeddable engine is **deny-by-default**. In embedded environments, host capabilities (`FileRead`, `FileWrite`, `NetConnect`, `Env`, `Run`) must be explicitly granted by the Rust host.
