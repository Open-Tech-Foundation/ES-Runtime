...
3. **Capability & Sandboxing Model**:
   - esrun is **deny-by-default**: a run reaches only what the command line named. `--allow-read`, `--allow-net`, `--allow-env`, `--allow-run` and the rest are how a service states what it may touch — see [Securing the runtime](/docs/guides/securing-runtime).
