...
What actually happens to a connection between `accept` and `close`, which limit
applies at each stage, why each default is the number it is, and what a
connection costs while it is open.
...
listing signatures — for those, see the [`runtime:http` reference](/api/http) —
and it covers the **inbound** server only. Where a decision had a real
alternative, the reasoning is recorded in `docs/DECISIONS.md` (D42–D45).
...
## The handoff: many connections, one isolate
