...
async function bundle(testPath, meta, mode) {
  ...
  for (const spec of meta.scripts) parts.push(await read(resolveScript(spec, testPath)));
  parts.push(await read(testPath));
...
const totals = { PASS: 0, FAIL: 0, TIMEOUT: 0, NOTRUN: 0, PRECONDITION_FAILED: 0 };
const harnessErrors = [];
const skipped = [];
const results = {};
const generated = [];
...
console.log(`${tests.length} test files under ${ROOTS.join(", ")}\n`);
...
for (const testPath of tests) {
  ...
  for (const mode of modes) {
    ...
    for (const t of report.tests) {
      const status = TEST_STATUS[t.status] ?? "FAIL";
      totals[status] += 1;
      subtests[t.name] = status;
      if (flags.verbose && status !== "PASS") {
    ...
    const passed = report.tests.filter((t) => t.status === 0).length;
    const label = harness === "OK" ? `${passed}/${report.tests.length}` : harness;
...
const run = totals.PASS + totals.FAIL + totals.TIMEOUT + totals.NOTRUN + totals.PRECONDITION_FAILED;
console.log(
  `\n${totals.PASS}/${run} subtests passing ` +
    `(${totals.FAIL} fail, ${totals.TIMEOUT} timeout, ${totals.NOTRUN} not run, ` +
    `${totals.PRECONDITION_FAILED} precondition) across ${Object.keys(results).length} runs`,
);
if (harnessErrors.length > 0) {
  console.log(`${harnessErrors.length} file(s) failed before their tests could report:`);
