...
async function bundle(testPath, meta, mode) {
  ...
  // WPT's generated worker wrapper (`x.any.worker.js`) ends with `done()`, and
  // the harness in a worker requires it: a dedicated worker has no "load" event
  // to end the run, so it waits to be told the test file has finished adding
  // tests. `.worker.js` files call it themselves; a second call is harmless.
  // Not in main mode, where the harness completes on its own — exactly the
  // split upstream's own wrappers make.
  if (mode === "worker") parts.push("done();");
...
async function runInWorker(bundleUrl, timeoutMs) {
  ...
  worker.onerror = (event) => {
    // Claimed, so it is not reported a second time on the way out — but *not*
    // treated as the end of the file. A test that starts workers of its own
    // (the module-worker suite does, some of them deliberately broken) sends
    // their failures here, and one failing subtest must not discard the results
    // of the other fifteen. It only becomes the answer if nothing else reports.
    event.preventDefault();
    if (!failure) failure = event.message;
  };
  const timer = deadline(timeoutMs);
  const report = await Promise.race([reported, timer.promise]);
  timer.cancel();
  ...
  return {
    harness: { status: failure ? 1 : 2, message: failure || "no result before the deadline" },
    tests: [],
  };
}
