...
import { connect, DbErrorCode, queryAst, sql } from "runtime:db";
...
is(
  (await db.execute(queryAst(["SET", "k", "v"]))).lastInsertRowid,
  null,
...
is(
  (await db.execute(queryAst(["SET"]), ["p", "1"])).changes,
  1,
  "positional arguments are appended",
);
is(
  await (await db.query(queryAst(["GET"]), ["p"])).first().then((r) => r.value),
  "1",
...
{
  is(
    await (await db.query(queryAst(["GET", "absent"]))).first(),
    null,
...
}
{
  ...
  is(
    (await rows.toArray()).map((r) => r.toObject()),
    [
      { field: "one", value: "1" },
      { field: "two", value: "2" },
    ],
...
// there was never a cursor to abandon.
{
  ...
  await db.execute(
    queryAst(["RPUSH", "big", ...Array.from({ length: 5000 }, (_, i) => String(i))]),
  );
  let seen = 0;
  ...
  is(
    (await (await db.query(queryAst(["LLEN", "big"]))).first()).value,
    5000,
...

{
  ...
  is(
    (await (await db.query(queryAst(["MGET", "a", "b", "c"]))).toArray()).map((r) => r.value),
    ["1", "2", "3"],
...
is(
  await codeOf(() => db.transaction(async () => {})),
...
is(
  await codeOf(() => db.execute(queryAst(["SET", "k"]), { name: "v" })),
  DbErrorCode.Unsupported,
  "an object of named parameters is refused",
);
is(
  await codeOf(() => db.dialect.placeholder(1)),
  DbErrorCode.QueryForm,
...
is(
  await codeOf(() => db.execute(queryAst(["SUBSCRIBE", "channel"]))),
  DbErrorCode.Unsupported,
  "SUBSCRIBE is refused by name",
);
is(
  await codeOf(() => db.execute(queryAst(["MONITOR"]))),
  DbErrorCode.Unsupported,
  "and so is MONITOR",
);
...
is(
  await codeOf(() => db.query(queryAst(["BLPOP", "q", "0"]))),
  DbErrorCode.Unsupported,
  "an unbounded BLPOP is refused",
);
is(
  await codeOf(() => db.query(queryAst(["XREAD", "BLOCK", "0", "STREAMS", "s", "$"]))),
  DbErrorCode.Unsupported,
  "and an unbounded XREAD",
);
{
  ...
  is(
    (await db.execute(queryAst(["SET", "after-refusal", "1"]))).changes,
    1,
...

{
  ...
  const rows = await (
    await db.query(queryAst(["GET", "a"]), [], { signal: quiet.signal })
  ).toArray();
  is(rows.length, 1, "an unaborted signal changes nothing");
  ok(
    (await (await db.query(queryAst(["GET", "a"]))).first()) !== null,
...
is(
  await codeOf(() => db.query(queryAst(["GET", "a"]))),
  DbErrorCode.Closed,
