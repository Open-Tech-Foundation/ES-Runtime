...
ok(
  parseConnectionString("rediss://localhost").tls === true,
  "rediss:// is TLS from the first byte",
);
...
is(
  parseConnectionString("redis://host?connect_timeout=5").connectTimeout,
  5000,
  "connect_timeout is seconds",
);
...
is(
  parseConnectionString("redis://host?binary=0").binary,
  false,
  "?binary=0 turns it off explicitly",
);
is(parseConnectionString("redis://host").binary, undefined, "and it is unset by default");
is(
  parseConnectionString("redis://host?command_timeout=250").commandTimeout,
  250,
...
is(
  parseConnectionString("redis://host:6379", { port: 7000 }).port,
  7000,
  "options override the URL",
);
