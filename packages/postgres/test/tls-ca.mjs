...
if (!url || !ca) {
  ...
  try {
    ...
    console.log(
      "unnamed CA refused:",
      e.code === DbErrorCode.Unsupported || /certificate|unknown|Tls/i.test(e.message),
    );
  }

  const db = await connect(`${url}?sslmode=require`, { driver: postgres, sslRootCert: ca });
  const row = await (
    await db.query("SELECT ssl, version FROM pg_stat_ssl WHERE pid = pg_backend_pid()")
  ).first();
  ...
  const p = await (
    await preferred.query("SELECT ssl FROM pg_stat_ssl WHERE pid = pg_backend_pid()")
  ).first();
