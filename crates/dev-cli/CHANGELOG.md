...
- **A computed colour is a colour.** `getComputedStyle(el).color` answered `red`
  ...
  the line, each case checked against it: a **name** survives the declaration
  and resolves when computed (`style.color = "RED"` reads `red`), a **hex or
  legacy function** is already canonical in the declaration (`#fff` and
  `hsl(0, 100%, 50%)` both read `rgb(…)` there), `transparent` computes to
  `rgba(0, 0, 0, 0)`, `currentcolor` to the element's own computed `color` —
  inherited through the tree — and a **modern** colour keeps the space it was
  written in, because converting `oklch()` would be a wrong answer rather than a
  better one. CSS Color 4's space-separated syntax is accepted. All twenty-one
  colour properties are resolved, and only those.

  The 148 names and the conversion arithmetic come from `@opentf/std`'s
  `color()`, bundled with our own `esdev` into
  `crates/dev-cli/src/dom/std-color.js` (8.7 KB, generated by `tsr build` in
  `crates/dev-cli/js`, drift-checked in CI beside `serialization.js`). It is
  reachable only from `runtime:dom/colors`, which is reachable only from `esdev
  test --dom`: `esrun`'s crate graph does not include `dev-cli`, and `start`,
  `build` and the plugin host never register the DOM extension, so a dev server
...
- Methods installed through a descriptor batch are **writable** as well as
  ...
  `document.getSelection` moved from the instance to `Document.prototype`, where
  a browser has them, so a realm-wide stub can reach them.
...
- Add esdev DOM `Element.classList` token operations synchronized with `class`.
- Parse well-formed SVG foreign content in esdev DOM fragments, including SVG
...
- Add modern `document.createElementNS()` namespace identity and qualified-name support.
- Keep esdev DOM tree internals private so framework `_children` render bookkeeping
  does not break DOM collections.
...
- Make esdev DOM form reset restore dirty input values and checked states from
...
- Add esdev DOM `input[type="submit"]` activation and reflected per-submitter
  form settings.
- Add esdev DOM button and input reset-control activation.
...
- `esdev create` scaffolds the OTF Web starter set (`spa`, `fullstack`,
  ...
  chosen explicitly. Menus show proper names (`SPA`, `API`, `ReactJS`) while
  flags stay lowercase. `Esc` steps back to the previous question instead of
...
- `test --help` no longer claims every flag is also an `esdev.json` key:
