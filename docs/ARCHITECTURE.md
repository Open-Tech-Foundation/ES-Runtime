...
- **Capabilities:** deny-by-default tokens threaded from the embedder; no global escape hatches. Nine of the thirteen are **host-facing** (`Capability::HOST_FACING`) and carry a name in the permission vocabulary — `read, write, imports, net, listen, env, run, signals, workers` — shared by the CLI's `--allow-<name>` / `--deny-<name>` flags, the `runtime:process` `permissions` API, and the text of a denial (D38). **`esrun` and the embeddable library both grant nothing unless asked to** (D65); `esdev`, the development binary, grants everything, which is the one place the two binaries differ.
