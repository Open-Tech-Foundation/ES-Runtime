...
- ☑ `NetProvider` — TCP sockets + listener and **UDP datagram sockets** for `runtime:net`, gated on `Net`/`NetListen` (DECISIONS D28, D58; a datagram socket is checked against **both** — `bind` on `NetListen`, `send`/`connect` on `Net`). *(Phase 12.)*
...
11. ☑ **`runtime:` standard modules I** — ☑ the `runtime:` built-in scheme (served by the runtime, loader-independent; ops are the capability boundary) and ☑ **`runtime:process`** (`env` mutable-in-process / `args` / `cwd()` / `platform` OS-native / `exit(code=0)`), gated on `Capability::Env`, backed by the new `Process` provider (DECISIONS D26). ☑ **`runtime:path`** (pure; uses `cwd`/`platform`) and ☑ **`runtime:fs`** (async file ops, jailed, incl. `Glob`, `copy`, `realPath`, `readLink`, `symlink`, `truncate`, `chmod`, and jailed `makeTempDir`/`makeTempFile`) over the new `FileSystem` provider. `copy` names **both** `FileRead` and `FileWrite` — ops may now require more than one capability, since gating a read-and-write call on the write alone would let a guest duplicate a file it cannot see. Phase 11 is complete.
12. ☑ **`runtime:` standard modules II** — ☑ **`runtime:serialization`** (XML validation, XML parser, and XML builder backed natively by `quick-xml`). ☑ **`runtime:net`** (TCP sockets + listener provider, client/server TLS per the WinterTC Sockets API; DECISIONS D28, and **UDP datagram sockets** — `bind()`, messages rather than streams, multicast, and a socket checked against both `NetListen` and `Net`; DECISIONS D58). ☑ **`runtime:http`** (the HTTP **server** capstone — `serve((req) => res)` over the `HttpServerProvider` seam, bodies **streaming both directions** with backpressure; DECISIONS D31), including **TLS termination** (`secureTransport: "on"` with an inline PEM `cert`/`key` and `alpn`, sharing `runtime:net`'s server-side rustls setup) — so `request.url` reports `https:`, a bad cert fails the bind rather than each handshake, and a failed handshake ends only its own connection — and **HTTP/2**, negotiated per connection (ALPN `h2` over TLS, h2c by prior knowledge on a cleartext port) with the version invisible to the handler, **response trailers** (`withTrailers`/`trailersOf`, both versions — what native gRPC needs; DECISIONS D46), the **peer address** on the handler's second argument (`info.remoteAddr`, the socket peer only — forwarded headers are the deployment's to trust; DECISIONS D44), an optional **connection cap** (`maxConnections`, which holds excess connections rather than refusing them; DECISIONS D45), and **connection timeouts** on by default (handshake/first byte, request head — which is also HTTP/1.1's idle keep-alive limit — and HTTP/2 PING probing; DECISIONS D43). No HTTP/3. ☑ Streaming `fetch` request bodies (chunked upload with backpressure; DECISIONS D20). ☑ **`runtime:workers`** — durable workers: addressable, single-call-at-a-time state in the embedded SQLite engine, where a call's result is gated on its writes having committed, so what a caller was told survives a `SIGKILL` (DECISIONS D80). No new capability and no new Rust — the module is guest JavaScript over `runtime:db`, `runtime:fs` and `runtime:hashing`. ☑ **Alarms** on top of it (DECISIONS D81): a time stored beside a worker's state and indexed in the catalog, an opt-in scheduler a process starts, and a failing `alarm()` retried and then reported. ☑ **Collections** (DECISIONS D82): a table per declared collection, the document stored as structured clone with declared fields promoted to indexed columns, a query builder over them, one transaction across keys and documents, and a lazy schema migration that backfills a field declared late. ☑ **Shards** (DECISIONS D122): the classes' code on a fixed pool of `Worker`s placed by a hash of the id, the storage kept on the agent that holds the file lock, a heartbeat that terminates a shard stuck in a loop, and a lost shard costing only the call in flight.
...
**Non-goals (this repo):**
- No actor/process model, scheduler, preemption, mailboxes, supervisors — a separate project, not this one (D66). **(Unchanged by D80:** `runtime:workers`' durable workers are a *guest* value with a queue in front of them, written in JavaScript over `runtime:db` and the existing agent seam. Nothing in `crates/` gained a scheduler, nothing preempts, and no agent gained a mailbox — the runtime still does not decide when guest code runs.**)**
...
- **`runtime:net` UDP** → ☑ **implemented** (DECISIONS D58). `bind()` returns a `DatagramSocket` — `send`/`receive`/`connect`, `joinMulticast`/`leaveMulticast`, `addr`, `close()`, and async-iterable of `{ data, address, port }` messages — rather than web streams, because a datagram arrives whole and carries its own sender. Socket options (`reusePort`, `reuseAddress`, `broadcast`, `ttl`, `multicastTtl`, `multicastLoopback`) are set at the bind, with the address family choosing the v4 or v6 spelling of each. Gated on **both** capabilities: `NetListen` to bind (a port is how a process is reached) and `Net` per destination to send. The WinterTC Sockets proposal is TCP-only and has nothing to say here; the shape follows Deno's `listenDatagram` with a named message object. **Not in:** QUIC/HTTP/3, unix-domain datagram sockets, `IP_MULTICAST_IF`, and a receive that reports truncation.
...
- **ES module loading** — ☑ **implemented**: static `import`/`export`, **dynamic `import()`** (resolving with the module namespace after the imported module fully evaluates; shares instances with static imports via the realm module map), `import.meta.url`, **`import.meta.resolve`**, native top-level await, **local `file:` modules**, **JSON modules via `with { type: "json" }`** (transpiled natively), and **`node_modules` resolution for ES module packages** via the capability-checked `ModuleLoader` provider (DECISIONS D21, D22, D23). `exports`/`imports` resolution is complete (D40): conditions matched **in the manifest's own key order** with nested fall-through — asserting the standards-only set `import`/`default`, no runtime-branded key — **subpath patterns** (`"./*"`), array fallbacks, `null` targets that withdraw a subpath, **`imports`/`#private` specifiers** (a path, a pattern, or another package), **self-reference** by the package's own `name`, and target validation that refuses a `..`/`.`/`node_modules` segment after `*` substitution so a capture cannot walk out of its package. **`import.meta.resolve` resolves every specifier** (D41), including bare and `#private` ones: the loader's resolution is a synchronous core that the async `resolve` also calls, so `import.meta.resolve` and `import()` cannot disagree, and both pass the same root jail and import policy. It is gated on `FileSystem` like the loader, so a denied run learns nothing; a loader with no synchronous path (`ModuleLoader::resolve_sync` returning `None`) still throws a `TypeError` naming the specifier. Relative, absolute-path and absolute-URL specifiers resolve with no I/O and no existence check, exactly as Node's does. **Rejected by design:** CommonJS packages, remote (`http:`) modules, and `node:` builtins (§125).
