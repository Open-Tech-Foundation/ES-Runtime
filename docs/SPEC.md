...
- **Conformance:** ☑ a curated in-repo suite of spec-behaviour assertions over the implemented surface (`crates/runtime/conformance/*.js`), run by the `conformance_suite_passes` gate with a recorded, non-regressing pass-rate (`conformance/RESULTS.md` — currently 62/62). The full WPT harness (`testharness.js`) is a later addition; the curated suite is meant to trend up as coverage grows.
...
12. ◐ **`runtime:` standard modules II** — ☑ **`runtime:parsers`** (XML validation, XML parser, and XML builder backed natively by `quick-xml`). Remaining: `runtime:net` (sockets, listener provider) and `runtime:http` (HTTP **server** — the standalone capstone). Streaming request bodies.
...
- **`runtime:net` TLS** → implemented per the WinterTC Sockets API (DECISIONS D28). **In:** `secureTransport: "on"` client TLS (certificate verification, **SNI**, **ALPN** surfaced as `SocketInfo.alpn`), the `Socket.upgraded` flag, `startTls()` / `secureTransport: "starttls"` in-place upgrade, **server-side TLS termination on `listen`** (`{ secureTransport: "on", cert, key, alpn }`, cert/key inline so no capability beyond `NetListen`), `allowHalfOpen`, and the combined `"host:port"` `SocketInfo` shape. The TLS surface is complete.
...
- **`reportError` ErrorEvent dispatch** and **sub-millisecond `performance.now`** are minimal in Phase 4; full behavior lands with the event loop / clock refinements.
  - Phase 13 (diagnostics) spans `engine` (☑ stack/position + error-class preservation), `runtime` (typed errors today; ◐ stable string codes outstanding), and `runtime-cli` (☑ coherent error-block formatting + color).
