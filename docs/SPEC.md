...
- ☑ **Compression Streams**: `CompressionStream`/`DecompressionStream` for `gzip`, `deflate` (zlib), and `deflate-raw`, over stateful native flate2 contexts behind pure sync ops (no capability); decompression errors on corrupt input and trailing junk at write time and on truncation at flush; the transformer-cancel hook frees the native context on abort/cancel.
