...
- **Modules:** `import.meta.url`, `import.meta.resolve(specifier[, parent])` — pure URL resolution against the current module, or against `parent` (an absolute URL, string or `URL`; a directory when it ends in `/` — not standard, and in Node only behind `--experimental-import-meta-resolve`), with no I/O and no existence check. **Bare and `#private` specifiers resolve too**, through the module loader — useful for locating a file *inside* a dependency (a migration, a `.proto`, a template) whose install path you cannot hardcode. That reads `package.json` files, so it needs the same `FileSystem` grant an import does, and it obeys the same root jail and import policy; a denied run gets a `NotAllowedError` rather than a location. A relative `parent` is a `TypeError`. A module is its URL, query included: `./m.js?v=2` is a module of its own (D126).
