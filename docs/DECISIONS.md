...
**Consequences:** Broad coverage with a pure-Rust, cross-platform build; per-algorithm crates to track. The TLS backend (D20, rustls) is independent of this choice. **Scope:** Phase 7 shipped getRandomValues, randomUUID, and `subtle` digest/HMAC/AES-GCM. Phase 7b adds AES-CBC/CTR, HKDF/PBKDF2 derivation, and ECDSA/ECDH over P-256/384/521 (`p256`/`p384`/`p521`); RSA remains staged (SPEC §7).
