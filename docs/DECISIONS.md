...
**Context:** `import source m from "./m.wasm"` — the source phase of the WebAssembly ESM integration — hands back the *uninstantiated* `WebAssembly.Module`, so one file can be instantiated repeatedly with different import objects. Node ships it (Stability 1.2, release candidate); Deno and Bun do not. It was the planned follow-up to D33, and the v8 crate does expose the pieces: `ModuleRequest::get_phase`, `Module::instantiate_module2` with a `ResolveSourceCallback`, and `ModuleImportPhase`.
...
Weighed against that: the feature is release-candidate in the one runtime that has it, experimental in V8 itself (the crate's own callback docs say so), and absent from Deno and Bun — so shipping it buys parity with one competitor at the cost of an experimental parser flag in a runtime whose whole claim is a hardened boundary. That is a bad trade *today*; it is a good one once both phases are wired together in one change.

**Consequences:** `import source` stays a clean `SyntaxError`, which is exactly what Deno and Bun do — no crash, no half-feature. The finding is recorded in a comment at the flag site so the next attempt starts from the failure mode rather than rediscovering it. Revisit when the static and dynamic phases can land together, ideally once V8 unflags the proposal. **The higher-value next increment is not this**: it is WASI's filesystem (D34), which needs a *synchronous* capability-gated provider seam — WASI syscalls are synchronous and `FileSystem` is async today — and would make the `FileRead`/`FileWrite` gates and the D25 root jail apply to WASI preopens, giving genuinely enforced sandboxing where Node explicitly disclaims it. That seam is a provider-surface expansion on the scale of D31 and wants its own decision.
