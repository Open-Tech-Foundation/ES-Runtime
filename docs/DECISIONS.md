...
- **Local files only for v1.** Relative/absolute-path/`file:` specifiers; **bare specifiers and remote (`http:`) schemes are rejected** (consistent with the "no npm/CommonJS/`node:`" non-goal, SPEC §125). Dynamic `import()`, import attributes / JSON modules, and remote modules are deferred (SPEC §7).
